"""LangChain adapter for operation-based structured memory.
...
class StructuredMemoryMiddleware(AgentMiddleware):
    ...
    def before_model(self, state: AgentState, runtime: Any) -> dict[str, Any] | None:
        ...
        # Preserve the public verifier customization point while delegating the
        # framework-neutral transaction to StructuredMemoryService.
        ...
        if update_result.rejected_ops:
            logger.warning(
                "Memory updater produced rejected ops; keeping messages for retry: %s",
                update_result.rejected_ops,
            )
            return None

        if update_result.failure_reason == "verification_failed":
            logger.warning(
                "Memory update failed semantic verification; keeping messages for retry: %s",
                update_result.verification.errors if update_result.verification else [],
            )
            return None

        if not update_result.committed:
            logger.warning("Memory changed before updater commit; keeping messages for retry")
            return None
    ...
    def wrap_model_call(self, request: "ModelRequest[Any]", handler: Callable[..., Any]) -> Any:
        ...
        entries = self.memory_selector.select_for_answer(
            memory=self.memory,
            query=query,
