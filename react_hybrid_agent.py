"""LangChain ReAct agent with summarization plus mem0 long-term vector memory.
...
script. If mem0ai is not installed, the demo falls back to summarization only.
"""
...
from memory_agent import Mem0LongTermMemory
...
def _summary_middleware(summary_model: str) -> SummarizationMiddleware:
...
def build_agent(
...
) -> tuple[Any, LongTermMemoryMiddleware | None]:
    """Build a ReAct-style agent with summary compression and optional mem0 recall."""
    resolved_summary_model = summary_model or SUMMARY_MODEL
    middleware: list[Any] = [_summary_middleware(resolved_summary_model)]
    long_term_middleware: LongTermMemoryMiddleware | None = None

    try:
        long_term = Mem0LongTermMemory.from_local(data_dir=data_dir or MEM0_DATA_DIR)
    except ImportError:
        print(
            "mem0ai is not installed; install it with `pip install mem0ai>=2.0` "
            "to enable long-term vector memory. Running with summarization only."
        )
    else:
    ...
    return agent, long_term_middleware
...
def main() -> None:
    ...
    for prompt in PROMPTS:
        ...
        if middleware is not None:
            for hit in middleware.last_recalled:
                print(f"[long-term recall] {hit.text}")

    persisted = middleware.flush() if middleware is not None else 0
