...
1. `react_summary_agent.py` is the recommended first version. It uses
   ...
   tool-calling agent.
2. `demo_react.py` is an experimental structured-memory version. It replaces
   `SummarizationMiddleware` with `StructuredMemoryMiddleware`, which evicts
   old messages into operation-based memory entries.

Start with the first path unless you specifically need auditable memory entries,
superseded history, or custom eviction behavior.
