"""Production-like memory replay with observable costs."""
...
class OnlineSimulation:
    ...
    def __init__(
        ...
        *,
        ...
        answer_selector: MemorySelector,
        ...
        self.memory_service = StructuredMemoryService(
            memory=self.memory,
            updater=self.updater,
            policy=self.memory.policy or self.updater.policy,
            compactor=self.compactor,
            compact_min_active_entries=0,
        )
    ...
    def _evict_and_update(self) -> None:
        ...
        if not batch:
            return
        try:
            result = self.memory_service.update(batch)
        except UpdateFailed:
            return
    ...
    def run(self, exchanges: Iterable[TranscriptExchange]) -> dict[str, object]:
        for index, exchange in enumerate(exchanges, start=1):
            ...
            entries = self.answer_selector.select_for_answer(
                memory=self.memory,
                query=exchange.user,
                budget=self.answer_memory_budget,
            )
